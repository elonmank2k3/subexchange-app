...
const InviteFriendScreen = ({ navigation }) => {
  ...
  const onCopyReferralCode = async  () => {
    await Clipboard.setStringAsync("ElonMank01");
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.subContainer}>
       <Text style={{textAlign: 'justify', fontSize: 18}}>When your friends enter your referral code, they will receive 200 coins and 30 minutes VIP account for trial. You also receive bonus of 2000 coins.
       </Text>
      </View>
      <View style={[styles.subContainer]}>
       <View>
      ...
      >
        ...
        <View style={[{position: 'relative', alignItems: 'center', marginTop: 10}]}> 
          <Text style={{fontSize: 25, fontWeight: 'bold', color: GlobalStyles.primaryColor}}>ElonMank01</Text>
          {
            isCodeCopied ? 
